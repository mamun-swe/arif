...
const ProjectPageAds = () => {
    ...
    const [adsCodes, setAdsCodes] = useState('')
    const [currentAds, setCurrentAds] = useState([])
    const [uploadLoader, setUploadLoader] = useState(false);
    const [enable, setEnable] = useState({})
    let newLink = [];

    const uploadImage = async (uploadImageTxt) => {
    ...
    useEffect(() => {
        fetch(`${Backendurl}getLandingPageAds`)
            .then(res => res.json())
            .then(data => {
                const projectAds = data.find(item => item.category === 'project' && item.position === position.value)

                setTime({ value: `${projectAds?.interval}`, label: `${projectAds?.interval}ms` })
                setEnable({ value: `${projectAds?.enable}`, label: `${projectAds?.enable}` })
                setCurrentAds(projectAds)
    ...
    const intervalHandler = () => {
        ...
        fetch(`${Backendurl}interval`, {
            method: 'POST',
            headers: { 'content-type': 'application/json' },
            body: JSON.stringify({ id: currentAds?._id, interval: time.value, adsGallery: currentAds?.adsGallery, position: position.value, category: 'project', enable: enable.value })
        }).then(res => res.json())
            .then(data => {
                setRefresh(data.success)
                setLoader(false);
    ...
    return (
        <div className='container ads-banner'>
            <div className='row'>
                <div className='col-6'>
                    <label htmlFor="exampleFormControlTextarea1">Upload Project Ads Image</label>
