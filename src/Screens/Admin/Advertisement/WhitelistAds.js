...
const WhitelistAds = () => {
    ...
    useEffect(() => {
        fetch(`${Backendurl}getLandingPageAds`)
            .then(res => res.json())
            .then(data => {
                const whiteListAds = data.find(item => item.category === 'whitelist' && item.position === position.value)

                setTime({ value: `${whiteListAds?.interval}`, label: `${whiteListAds?.interval}ms` })
                setEnable({ value: `${whiteListAds?.enable}`, label: `${whiteListAds?.enable}` })
                setCurrentAds(whiteListAds)
    ...
    const intervalHandler = () => {
        ...
        fetch(`${Backendurl}interval`, {
            method: 'POST',
            headers: { 'content-type': 'application/json' },
            body: JSON.stringify({ id: currentAds?._id, interval: time.value, adsGallery: currentAds?.adsGallery, position: position.value, category: 'whitelist', enable: enable.value })
        }).then(res => res.json())
            .then(data => {
                setRefresh(data.success)
                setLoader(false);
    ...
    return (
        <div className='container ads-banner'>
            <div className='row'>
                <div className='col-6'>
                    <label htmlFor="exampleFormControlTextarea1">Upload Whitelist Ads Image</label>
